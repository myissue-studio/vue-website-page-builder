...
const insertFormattedText = async (
  ...
  const replace = options?.replacePage === true
  ...
  closeAddComponentModal()
  showToast(formatAddedBlocksMessage(translate, count, { replacedPage: replace }), 'success')
}
...
const applyHelperButtonStyle = (componentObject: ComponentObject): ComponentObject => {
  ...
  buttonBlocks.forEach((block) => {
    ...
    link.classList.remove(
      ...
      'pbx-text-myPrimaryButtonTextColor',
      'pbx-text-white',
      'pbx-inline-flex',
      'pbx-items-center',
      'pbx-justify-center',
    ...
    if (helperButtonStyle.value === 'button') {
      link.classList.add('pbx-bg-myPrimaryButtonColor', 'pbx-text-myPrimaryButtonTextColor')
...
<template>
    ...
    <div v-if="!isLoading">
      ...
      <div class="pbx-modalFilterBar">
        ...
        <div class="pbx-modalFilterBarRow">
          ...
          <ModalFilterChip
            icon="content_paste"
            :label="translate('Paste your text')"
            :hint="translate('Paste from Word, Docs, or ChatGPT')"
  ...
  <InsertFormattedTextModal
    :open="showFormattedTextModal"
    :show-replace-toggle="hasPageContent"
    @close="closeFormattedTextModal"
    @insert="insertFormattedText"
  />
</template>
